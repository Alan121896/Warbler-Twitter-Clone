...
def signup():
    ...
    if form.validate_on_submit():
        try:
            ...
            '''testing out pdb and model relationships via the terminal'''
...
#adding like and unlike features
...
def user_likes(user_id):
    ...
    user = User.query.get_or_404(user_id)
    liked_messages = [like.message for like in user.likes]  # This fetches all the liked messages
...
def homepage():
    ...
    if g.user:
        ...
        #get the ID of the users the current user is following
        following_ids = [user.id for user in g.user.following]

        #include  the current user ID to show their own warbles
        following_ids.append(g.user.id)

        #fetch the latest 100 messages only from the users in following_ids
        messages = (Message
                    .query
        ...
        liked_msg_ids = {like.message_id for like in g.user.likes}
